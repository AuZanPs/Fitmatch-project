...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: basicCleanupData, error: basicCleanupError } =
      await supabase.rpc("cleanup_old_cache", { age_interval: "30 days" });
    ...
    if (basicCleanupError) {
      ...
      try {
        const { data: directCleanupData, error: directCleanupError } =
          await supabase
    ...
    const { data: smartCleanupData, error: smartCleanupError } =
      await supabase.rpc("smart_cache_cleanup", {
    ...
    if (smartCleanupError) {
      ...
      console.log(
        "Smart cleanup completed, additional entries deleted:",
        smartCleanupData,
      );
    }
    ...
    try {
      ...
      if (sizeData) {
        ...
        if (sampleData && sampleData.length > 0) {
          ...
          const avgRowSizeBytes =
            sampleData.reduce((sum, row) => {
          ...
          const estimatedTotalSizeMB =
            (avgRowSizeBytes * (entryCount || 0)) / (1024 * 1024);
