...
import { Plus, LogOut, Upload, Sparkles, Grid3X3, List, Filter, Search, Trash2, X } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [selectedImageModal, setSelectedImageModal] = useState<ClothingItemWithTags | null>(null);
        ...
        >
          <div className="relative max-w-6xl max-h-full w-full mx-2">
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] sm:max-h-[90vh] flex flex-col lg:flex-row">
              {/* Image Section */}
              <div className="flex-1 bg-gray-50 flex items-center justify-center p-3 sm:p-6 lg:p-8 min-h-[45vh] lg:min-h-0" onClick={(e) => e.stopPropagation()}>
                <OptimizedImage
              ...
              <div className="w-full lg:w-80 bg-white p-4 sm:p-6 lg:p-8 flex flex-col justify-between max-h-[45vh] lg:max-h-none overflow-y-auto lg:overflow-visible">
                <div>
                  ...
                  {selectedImageModal.color && (
                    <div className="mb-6">
                      <h3 className="font-playfair text-sm font-semibold text-black mb-3">Color</h3>
                  ...
                  {/* Style Tags */}
                  {selectedImageModal.clothing_item_style_tags && selectedImageModal.clothing_item_style_tags.length > 0 && (
                    <div className="mb-6">
                      <h3 className="font-playfair text-sm font-semibold text-black mb-3">Style Tags</h3>
                      <div className="flex flex-wrap gap-2">
                        {selectedImageModal.clothing_item_style_tags.map((tagRelation) => (
                          <span
            ...
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
              ...
              <div className="hidden md:flex items-center gap-2 text-xs sm:text-sm text-mejiwoo-gray font-montserrat">
                ...
                {selectedStyle && (
                  <>
                    <span>•</span>
                    <span className="truncate">{filteredItems.length} {selectedStyle.name.toLowerCase()}</span>
                  </>
                )}
              </div>
            </div>
        ...
        {showFilters && (
          <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-playfair text-base sm:text-lg font-semibold text-black">Filters & Search</h3>
              <button
              ...
              >
                ...
                <span className="hidden sm:inline">Back to Simple Search</span>
            ...
            {/* Style Tags Filter */}
            <div>
              <h4 className="font-inter text-xs sm:text-sm font-medium text-black mb-3">Filter by Style</h4>
              <div className="flex flex-wrap gap-2">
                <button
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const matchingStyle = styleTags.find(style => 
                    style.name.toLowerCase().includes(e.target.value.toLowerCase().trim())
                  );
                  if (e.target.value.trim() === '') {
                    handleStyleChange(null); // Show all items when empty
                  } else if (matchingStyle && e.target.value.toLowerCase() === matchingStyle.name.toLowerCase()) {
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    const matchingStyle = styleTags.find(style => 
                      style.name.toLowerCase().includes(styleSearchQuery.toLowerCase().trim())
                    );
                    if (matchingStyle) {
                      handleStyleChange(matchingStyle);
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-20 max-h-60 overflow-y-auto">
                  ...
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-100 ${
                      !selectedStyle ? 'bg-black text-white hover:bg-gray-800' : ''
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium">All Items</span>
                      <span className={`text-sm ${!selectedStyle ? 'text-gray-300' : 'text-gray-500'}`}>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${
                            selectedStyle?.id === style.id ? 'bg-black text-white hover:bg-gray-800' : ''
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <span className="font-medium">{style.name}</span>
                            <span className={`text-sm ${
                              selectedStyle?.id === style.id ? 'text-gray-300' : 'text-gray-500'
                            }`}>
                              {itemCount} items
                            </span>
                          </div>
                          ...
                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-200">
                            <div className="absolute bottom-0 left-0 right-0 p-1.5 sm:p-2 lg:p-3 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {item.clothing_item_style_tags?.slice(0, 1).map((tagRelation) => (
                                  <span
                                    key={tagRelation.style_tag.id}
                                ...
                                {item.clothing_item_style_tags && item.clothing_item_style_tags.length > 1 && (
                                  <span className="text-xs bg-white/20 text-white px-1.5 py-0.5 rounded-full">
                                    +{item.clothing_item_style_tags.length - 1}
                                  </span>
                                )}
                              </div>
                            </div>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2 z-40">
        <div className="flex items-center justify-around">
          ...
          >
            ...
            <span className="text-xs font-montserrat text-purple-600">AI</span>
          </button>
          ...
          {allItems.length > 0 && (
            ...
            >
              ...
              <span className="text-xs font-montserrat text-red-600">Manage</span>
            </button>
          )}
          ...
          <button
            ...
            className="flex flex-col items-center gap-1 p-2 rounded-lg hover:bg-gray-50 transition-colors"
          >
            <Filter className={`w-5 h-5 ${showFilters ? 'text-black' : 'text-gray-600'}`} />
            <span className={`text-xs font-montserrat ${showFilters ? 'text-black' : 'text-gray-600'}`}>
